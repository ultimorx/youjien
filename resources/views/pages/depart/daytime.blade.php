...
@section('scripts')
<script src="{{asset('js/daytime.js')}}?{{filemtime('js/daytime.js')}}"></script>
@endsection
...
<div class="row position-relative" id="children-list" v-cloak>
    <div class="position-absolute fixed-right d-print-none">
        <button type="button" class="btn btn-secondary city_hidden" @click="check()" v-if="rosters.length != 0">すべてチェックする</button>
        <button type="button" class="btn btn-primary city_hidden" @click="depart()" v-if="rosters.length != 0">チェックしたものを降園にする</button>
    </div>
    <table class="table table-striped sheet">
        <caption>
            お迎え一覧　@{{rosters.length}}人
        </caption>
        <thead>
            <tr>
        ...
        <tbody class="text-center">
            <tr v-for="(roster, index) in rosters">
                <td class="text-left">
